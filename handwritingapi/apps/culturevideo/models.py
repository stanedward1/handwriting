...
class CultureVideoCategory(BaseModel):
    """分类"""
    name = models.CharField(max_length=64, unique=True, verbose_name="文化趣事视频分类")

    class Meta:
        db_table = "culture_video_category"
        verbose_name = "文化趣事视频分类"
...
class Video(BaseModel):
    ...
    attachment_path = models.FileField(upload_to="attachment", max_length=128, verbose_name="附件路径", blank=True,
                                       null=True)
    video_category = models.ForeignKey(CultureVideoCategory, on_delete=models.CASCADE,
                                       verbose_name="文化趣事视频分类")
    students = models.IntegerField(verbose_name="学习人数", default=0)
    sections = models.IntegerField(verbose_name="总课时数量", default=0)
    pub_sections = models.IntegerField(verbose_name="文化趣事视频更新数量", default=0)
    organization = models.ForeignKey("Organization", on_delete=models.DO_NOTHING, null=True, blank=True,
                                     verbose_name="机构或个人名称")

    class Meta:
        db_table = "culture_video_video"
        verbose_name = "文化趣事视频"
        verbose_name_plural = "文化趣事视频"

    @property
    def video_section(self):
        ll = []
        # 根据文化趣事视频取出所有章节
        video_chapter_list = self.culturevideochapters.all()
...
class Organization(BaseModel):
    ...
    class Meta:
        db_table = "culture_video_organization"
        verbose_name = "机构或个人名称"
        verbose_name_plural = verbose_name
...
class CultureVideoChapter(BaseModel):
    """文化趣事视频章节"""
    video = models.ForeignKey("Video", related_name='culturevideochapters', on_delete=models.CASCADE, verbose_name="文化趣事视频名称")
    ...
    class Meta:
        db_table = "culture_video_chapter"
        verbose_name = "文化趣事视频章节名称"
        verbose_name_plural = verbose_name
...
class CultureVideoSection(BaseModel):
    """每个小节"""
    section_type_choices = (
        (0, '文档'),
        (1, '文化趣事视频')
    )
    chapter = models.ForeignKey("CultureVideoChapter", related_name='culturevideosections', on_delete=models.CASCADE,
                                verbose_name="文化趣事视频章节")
    ...
    duration = models.CharField(verbose_name="文化趣事视频时长", blank=True, null=True, max_length=32)  # 仅在前端展示使用
    pub_date = models.DateTimeField(verbose_name="发布时间", auto_now_add=True)

    class Meta:
        db_table = "culture_video_Section"
        verbose_name = "小节标题"
        verbose_name_plural = verbose_name
